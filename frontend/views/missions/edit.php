...
if (!defined('MNS_CONTROLLER_RENDER')) {
    ...
    if (($_GET['action'] ?? '') === 'update') {
        ...
        $mission = findMissionOrRedirect((int) ($_GET['id'] ?? 0));
        $clients = $model->getActiveClients();
        $catalogItems = $model->getActiveCatalogItems();
        $flash = getFlashMessage();
        require_once __DIR__ . '/../includes/header.php';
        ?>
        <div class="d-flex"><?php require_once __DIR__ . '/../includes/sidebar.php'; ?><main class="container py-4"><?php if ($flash !== null): ?><div class="alert alert-danger"><?php echo e($flash['message']); ?></div><?php endif; ?><h1 class="h4 mb-3">Modifier la mission</h1><form method="post" action="/MNS_CORPORATE/frontend/views/missions/edit.php?action=update"><input type="hidden" name="id" value="<?php echo e((string) $mission['id']); ?>"><?php require __DIR__ . '/form.php'; ?><div class="mt-3"><button class="btn btn-primary" type="submit">Enregistrer</button><a class="btn btn-link" href="/MNS_CORPORATE/frontend/views/missions/list.php">Annuler</a></div></form></main></div><?php require_once __DIR__ . '/../includes/footer.php';
    }
    return;
}
