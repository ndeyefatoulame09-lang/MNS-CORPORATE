...
<main class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div><h1 class="h4 mb-1">Missions</h1><div class="text-muted"><?php echo e((string) ($total ?? 0)); ?> resultat(s)</div></div>
    ...
    <form class="row g-2 mb-3" method="get" action="/MNS_CORPORATE/frontend/views/missions/list.php">
        ...
        <div class="col-md-2"><select class="form-select" name="client_id"><option value="">Tous clients</option><?php foreach (($clients ?? []) as $client): ?><option value="<?php echo e((string) $client['id']); ?>" <?php echo (($filters['client_id'] ?? '') == $client['id']) ? 'selected' : ''; ?>><?php echo e($client['company_name']); ?></option><?php endforeach; ?></select></div>
        <div class="col-md-2"><select class="form-select" name="mission_catalog_id"><option value="">Tous types</option><?php foreach (($catalogItems ?? []) as $item): ?><option value="<?php echo e((string) $item['id']); ?>" <?php echo (($filters['mission_catalog_id'] ?? '') == $item['id']) ? 'selected' : ''; ?>><?php echo e($item['name']); ?></option><?php endforeach; ?></select></div>
